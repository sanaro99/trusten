...
function logReceived(data: any) {
  ...
  console.log(`${color}[${timestamp()}] ${icon} RECEIVED ← [${eventType.toUpperCase()}]${Colors.RESET}`)
...
async function testClient(): Promise<boolean> {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (CLIENT_TIMEOUT > 0) {
      ...
      logInfo('Client timeout disabled - will wait indefinitely for server response')
    }
    ...
    ws.onmessage = (event) => {
      try {
        ...
        logError(`Failed to parse message: ${error instanceof Error ? error.message : String(error)}`)
      }
    }
    ...
    ws.onclose = (event) => {
      ...
      console.log(`  Session ID:         ${sessionId ? sessionId.substring(0, 32) + '...' : 'N/A'}`)
      console.log(`  Events Received:    ${eventsReceived}`)
      console.log(`  Completion:         ${completionReceived ? '✅ YES' : '❌ NO'}`)
      console.log(`  Status:             ${completionReceived ? '✅ PASS' : '❌ FAIL'}`)
      console.log()
