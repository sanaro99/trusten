...
async function testClient(clientId: number, message: string): Promise<TestResult> {
      ...
      try {
        ...
        switch (data.type) {
          case 'connection':
            ...
            log(clientId, '📥', `Connection confirmed (${sessionId.substring(0, 8)}...)`)
          ...
          case 'response':
            log(clientId, '📥', `Response: ${data.content.substring(0, 80)}...`)
            break

          case 'tool_use':
            log(clientId, '📥', `Tool use: ${data.content.substring(0, 50)}...`)
            break
          ...
          case 'completion':
            log(clientId, COLORS.green + '📥' + COLORS.reset, 'Completion received')
          ...
          case 'error':
            log(clientId, COLORS.red + '❌' + COLORS.reset, `Error: ${data.error}`)
            break
        }
      } catch (error) {
        log(clientId, COLORS.red + '❌' + COLORS.reset, 'Failed to parse message')
      }
    }
...
async function testCapacityLimit(): Promise<boolean> {
  console.log(COLORS.yellow + '\n━━━━━━━━━━━ CAPACITY LIMIT TEST ━━━━━━━━━━━' + COLORS.reset)
  ...
  return new Promise((resolve) => {
    ...
    ws.onopen = () => {
      console.log(COLORS.red + '❌ 6th client connected (should have been rejected!)' + COLORS.reset)
      ws.close()
      resolve(false)
    }

    ws.onerror = () => {
      console.log(COLORS.green + '✅ 6th client rejected as expected' + COLORS.reset)
      resolve(true)
    }
    ...
    setTimeout(() => {
      if (ws.readyState !== WebSocket.OPEN) {
        console.log(COLORS.green + '✅ 6th client rejected (connection failed)' + COLORS.reset)
        resolve(true)
      }
    }, 2000)
  })
...
async function runTests() {
  console.log(COLORS.cyan + '━━━━━━━━━━━━━━━━━━━━ MULTI-CLIENT TEST ━━━━━━━━━━━━━━━━━━━━' + COLORS.reset)
  console.log()

  // Test 1: Concurrent execution with 3 clients
  console.log(COLORS.yellow + '━━━━━━━━━━━ TEST 1: CONCURRENT EXECUTION ━━━━━━━━━━━' + COLORS.reset)
  ...
  const clients = [
    ...
    testClient(2, 'I am client 2. open youtube.com and tell me the page title.'),
    testClient(3, 'I am client 3. open facebook.com and tell me the page title.'),
    testClient(4, 'I am client 4. open github.com and tell me the page title.'),
    testClient(5, 'I am client 5. open twitter.com and tell me the page title.'),
    testClient(6, 'I am client 6. open reddit.com and tell me the page title.')
  ]

  const results = await Promise.all(clients)

  console.log('\n' + COLORS.yellow + '━━━━━━━━━━━ TEST 1 RESULTS ━━━━━━━━━━━' + COLORS.reset)
  const allSuccessful = results.every(r => r.success)
  results.forEach(result => {
    const status = result.success ? COLORS.green + '✅ PASS' : COLORS.red + '❌ FAIL'
    console.log(`${status}${COLORS.reset} - Client ${result.clientId}: ${result.eventsReceived} events, completion: ${result.completionReceived}`)
  })
  ...
  console.log(COLORS.yellow + '\n━━━━━━━━━━━ TEST 2: CAPACITY LIMIT (SKIPPED) ━━━━━━━━━━━' + COLORS.reset)
  console.log('To test capacity: Set MAX_SESSIONS=3, run 3 clients, then try 4th')
  ...
  console.log('\n' + COLORS.cyan + '━━━━━━━━━━━━━━━━━━━━ TESTS COMPLETE ━━━━━━━━━━━━━━━━━━━━' + COLORS.reset)
