...
export function formatUIMessageStreamEvent(event: UIMessageStreamEvent): string {
...
export class UIMessageStreamWriter {
  ...
  async writeTextDelta(delta: string): Promise<void> {
    ...
    if (this.currentTextId === null) {
      ...
      await this.write(formatUIMessageStreamEvent({ type: 'text-start', id: this.currentTextId }));
    }

    await this.write(formatUIMessageStreamEvent({ type: 'text-delta', id: this.currentTextId, delta }));
  }

  async endText(): Promise<void> {
    if (this.currentTextId !== null) {
      await this.write(formatUIMessageStreamEvent({ type: 'text-end', id: this.currentTextId }));
  ...
  async writeReasoningDelta(delta: string): Promise<void> {
    ...
    if (this.currentReasoningId === null) {
      ...
      await this.write(formatUIMessageStreamEvent({ type: 'reasoning-start', id: this.currentReasoningId }));
    }

    await this.write(formatUIMessageStreamEvent({ type: 'reasoning-delta', id: this.currentReasoningId, delta }));
  }

  async endReasoning(): Promise<void> {
    if (this.currentReasoningId !== null) {
      await this.write(formatUIMessageStreamEvent({ type: 'reasoning-end', id: this.currentReasoningId }));
      this.currentReasoningId = null;
    }
  }

  async writeToolCall(toolCallId: string, toolName: string, input: unknown): Promise<void> {
    if (!this.hasStartedStep) await this.startStep();
    await this.endText();
  ...
  async writeToolError(toolCallId: string, errorText: string, isInput = false): Promise<void> {
    if (isInput) {
      await this.write(formatUIMessageStreamEvent({ type: 'tool-input-error', toolCallId, errorText }));
    } else {
      await this.write(formatUIMessageStreamEvent({ type: 'tool-output-error', toolCallId, errorText }));
    }
  }
