...
export class BookmarkAdapter {
  ...
  async getBookmarkTree(): Promise<chrome.bookmarks.BookmarkTreeNode[]> {
    ...
    try {
      ...
      logger.debug(`[BookmarkAdapter] Retrieved bookmark tree with ${tree.length} root nodes`);
      return tree;

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`[BookmarkAdapter] Failed to get bookmark tree: ${errorMessage}`);
  ...
  async searchBookmarks(query: string): Promise<chrome.bookmarks.BookmarkTreeNode[]> {
    ...
    try {
      ...
      logger.debug(`[BookmarkAdapter] Found ${results.length} bookmarks matching "${query}"`);
      return results;

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`[BookmarkAdapter] Failed to search bookmarks: ${errorMessage}`);
   ...
   */
  async createBookmark(bookmark: { title: string; url: string; parentId?: string }): Promise<chrome.bookmarks.BookmarkTreeNode> {
    logger.debug(`[BookmarkAdapter] Creating bookmark: ${bookmark.title || 'Untitled'}`);

    try {
      const created = await chrome.bookmarks.create(bookmark);
      logger.debug(`[BookmarkAdapter] Created bookmark: ${created.id} - ${created.title}`);
      return created;

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`[BookmarkAdapter] Failed to create bookmark: ${errorMessage}`);
  ...
  async removeBookmark(id: string): Promise<void> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`[BookmarkAdapter] Failed to remove bookmark ${id}: ${errorMessage}`);
  ...
  async updateBookmark(
    ...
    try {
      ...
      logger.debug(`[BookmarkAdapter] Updated bookmark: ${id} - ${updated.title}`);
      return updated;

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`[BookmarkAdapter] Failed to update bookmark ${id}: ${errorMessage}`);
  ...
  async getRecentBookmarks(limit: number = 20): Promise<chrome.bookmarks.BookmarkTreeNode[]> {
    ...
    try {
      ...
      logger.debug(`[BookmarkAdapter] Found ${urlBookmarks.length} recent bookmarks`);
      return urlBookmarks;

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`[BookmarkAdapter] Failed to get recent bookmarks: ${errorMessage}`);
   ...
   * @private
   */
  private _flattenBookmarkTree(nodes: chrome.bookmarks.BookmarkTreeNode[]): chrome.bookmarks.BookmarkTreeNode[] {
