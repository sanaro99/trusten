...
const mcpGetInstances: ToolDefinition = {
  ...
  description:
    'Get all installed Klavis MCP servers (Gmail, Google Calendar, Google Sheets, Google Docs, Notion, Slack, GitHub, etc.) with their instance IDs and authentication status. REQUIRED: Must provide userId parameter (e.g., userId: "nxtscape_1762796669049_ql48ymi8g").',
  annotations: {
    category: 'mcp',
    readOnlyHint: true,
  },
  schema: {
    userId: z.string().optional().describe('Klavis user ID (e.g., "nxtscape_1762796669049_ql48ymi8g")'),
  },
  handler: async (request, response, _context) => {
    ...
    if (!userId) {
      throw new Error(
        'userId is required for Klavis MCP tools. Please provide your Klavis user ID. Example: { "userId": "nxtscape_1762796669049_ql48ymi8g" }',
      );
    }
...
const mcpListTools: ToolDefinition = {
  name: 'mcp_list_tools',
  description:
    'List available tools for a specific Klavis MCP server instance (e.g., list all Gmail tools like send_email, read_email, search_emails). Requires instanceId from mcp_get_instances and userId from client.',
  annotations: {
    category: 'mcp',
    readOnlyHint: true,
  },
  ...
  handler: async (request, response, _context) => {
    ...
    if (!userId) {
      throw new Error(
        'userId is required for Klavis MCP tools. Please provide your Klavis user ID. Example: { "instanceId": "...", "userId": "nxtscape_1762796669049_ql48ymi8g" }',
      );
    }
...
const mcpCallTool: ToolDefinition = {
  name: 'mcp_call_tool',
  description:
    'Execute a tool on a Klavis MCP server (e.g., send Gmail email, create Google Calendar event, read Notion pages, post to Slack, etc.). Requires instanceId, toolName, toolArgs, and userId from client.',
  annotations: {
    category: 'mcp',
    readOnlyHint: false,
  },
  ...
  handler: async (request, response, _context) => {
    ...
    if (!userId) {
      throw new Error(
        'userId is required for Klavis MCP tools. Please provide your Klavis user ID. Example: { "instanceId": "...", "toolName": "...", "userId": "nxtscape_1762796669049_ql48ymi8g" }',
      );
    }
