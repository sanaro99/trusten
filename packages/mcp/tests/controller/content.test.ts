...
describe('MCP Controller Content Tools', () => {
  describe('browser_get_page_content - Success Cases', () => {
    it(
      'tests that page content extraction with text type succeeds',
      async () => {
    ...
    it(
      'tests that page content extraction with text-with-links type succeeds',
      async () => {
        await withMcpServer(async client => {
          ...
          if (!result.isError) {
            assert.ok(
              textContent.text.includes('Total pages:') || textContent.text.includes('Error:'),
    ...
    it(
      'tests that page content extraction with specific page number succeeds',
      async () => {
        await withMcpServer(async client => {
          // Navigate to a page with content
          const navResult = await client.callTool({
          ...
          if (!result.isError) {
            assert.ok(
              textContent.text.includes('Page 1 of') || textContent.text.includes('Error:'),
    ...
    it(
      'tests that page content extraction with all pages succeeds',
      async () => {
        await withMcpServer(async client => {
          const navResult = await client.callTool({
            name: 'browser_navigate',
    ...
    it(
      'tests that page content extraction with different context window sizes succeeds',
      async () => {
        await withMcpServer(async client => {
          const navResult = await client.callTool({
            name: 'browser_navigate',
          ...
          for (const contextWindow of contextWindows) {
            ...
            if (!result.isError) {
              assert.ok(
                textContent.text.includes(contextWindow) || textContent.text.includes('Error:'),
    ...
    it(
      'tests that empty page content extraction is handled',
      async () => {
        await withMcpServer(async client => {
          const navResult = await client.callTool({
            name: 'browser_navigate',
  ...
  describe('browser_get_page_content - Error Handling', () => {
    it(
      'tests that content extraction with invalid tab ID is handled',
      async () => {
        await withMcpServer(async client => {
          const result = await client.callTool({
            name: 'browser_get_page_content',
    ...
    it(
      'tests that invalid type enum is rejected',
      async () => {
        await withMcpServer(async client => {
          const navResult = await client.callTool({
            name: 'browser_navigate',
    ...
    it(
      'tests that invalid page number is handled',
      async () => {
        await withMcpServer(async client => {
          const navResult = await client.callTool({
            name: 'browser_navigate',
    ...
    it(
      'tests that non-numeric page number is handled',
      async () => {
        await withMcpServer(async client => {
          const navResult = await client.callTool({
            name: 'browser_navigate',
  ...
  describe('browser_get_page_content - Response Structure Validation', () => {
    it(
      'tests that content tool returns valid MCP response structure',
      async () => {
        await withMcpServer(async client => {
          const navResult = await client.callTool({
            name: 'browser_navigate',
  ...
  describe('browser_get_page_content - Workflow Tests', () => {
    it(
      'tests complete content extraction workflow: navigate -> extract text -> extract text-with-links',
      async () => {
        await withMcpServer(async client => {
          // Navigate to a page
          const navResult = await client.callTool({
    ...
    it(
      'tests pagination workflow: extract all pages -> extract specific page',
      async () => {
        await withMcpServer(async client => {
          const navResult = await client.callTool({
            name: 'browser_navigate',
            arguments: {
              url: 'data:text/html,<body><h1>Long Content</h1><p>'.repeat(100) +
                'Content paragraph.' +
                '</p>'.repeat(100) +
                '</body>',
            },
          ...
          assert.ok(!allPagesResult.isError, 'All pages extraction should succeed');
