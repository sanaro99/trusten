...
describe('MCP Controller Screenshot Tool', () => {
  describe('browser_get_screenshot - Success Cases', () => {
    it(
      'tests that screenshot capture with default settings succeeds',
      async () => {
    ...
    it(
      'tests that screenshot capture with small size preset succeeds',
      async () => {
        await withMcpServer(async client => {
          // Navigate to a page
          const navResult = await client.callTool({
    ...
    it(
      'tests that screenshot capture with medium size preset succeeds',
      async () => {
        await withMcpServer(async client => {
          // Navigate to a page
          const navResult = await client.callTool({
    ...
    it(
      'tests that screenshot capture with large size preset succeeds',
      async () => {
        await withMcpServer(async client => {
          // Navigate to a page
          const navResult = await client.callTool({
    ...
    it(
      'tests that screenshot capture with custom width and height succeeds',
      async () => {
        await withMcpServer(async client => {
          // Navigate to a page
          const navResult = await client.callTool({
    ...
    it(
      'tests that screenshot capture with showHighlights enabled succeeds',
      async () => {
        await withMcpServer(async client => {
          // Navigate to a page
          const navResult = await client.callTool({
  ...
  describe('browser_get_screenshot - Error Handling', () => {
    it(
      'tests that screenshot of invalid tab ID is handled',
      async () => {
        await withMcpServer(async client => {
          const result = await client.callTool({
            name: 'browser_get_screenshot',
    ...
    it(
      'tests that screenshot with non-numeric tab ID is rejected',
      async () => {
        await withMcpServer(async client => {
          try {
    ...
    it(
      'tests that screenshot with invalid size preset is rejected',
      async () => {
        await withMcpServer(async client => {
    ...
    it(
      'tests that screenshot with negative dimensions is rejected',
      async () => {
        await withMcpServer(async client => {
          // Navigate to a page first
          const navResult = await client.callTool({
  ...
  describe('browser_get_screenshot - Response Structure Validation', () => {
    it(
      'tests that screenshot tool returns valid MCP response structure',
      async () => {
        await withMcpServer(async client => {
          // Navigate to a page
          const navResult = await client.callTool({
          ...
          for (const item of result.content) {
            ...
            if (item.type === 'image') {
              assert.ok(
                'data' in item,
                'Image content must have data property',
              );
              assert.ok(
                'mimeType' in item,
                'Image content must have mimeType',
              );
              assert.strictEqual(
                typeof item.data,
                'string',
  ...
  describe('browser_get_screenshot - Workflow Tests', () => {
    it(
      'tests complete screenshot workflow: navigate, multiple screenshots with different sizes',
      async () => {
        await withMcpServer(async client => {
          // Navigate to a page
          const navResult = await client.callTool({
