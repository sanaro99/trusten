...
describe('MCP Controller Navigation Tools', () => {
  describe('browser_navigate - Success Cases', () => {
    it(
      'tests that navigation to HTTPS URL succeeds',
      async () => {
    ...
    it(
      'tests that navigation to data URL succeeds',
      async () => {
        await withMcpServer(async client => {
          const result = await client.callTool({
            name: 'browser_navigate',
          ...
          assert.ok(
            !result.isError,
            'Navigation to data URL should succeed',
          );
    ...
    it(
      'tests that navigation to HTTP URL succeeds',
      async () => {
        await withMcpServer(async client => {
          const result = await client.callTool({
            name: 'browser_navigate',
  ...
  describe('browser_navigate - Error Handling', () => {
    it(
      'tests that invalid URL is handled gracefully',
      async () => {
        await withMcpServer(async client => {
          const result = await client.callTool({
            name: 'browser_navigate',
    ...
    it(
      'tests that meaningful response structure is provided on any error',
      async () => {
        await withMcpServer(async client => {
  ...
  describe('browser_navigate - Response Structure Validation', () => {
    it(
      'tests that valid MCP response structure is always returned',
      async () => {
        await withMcpServer(async client => {
          const result = await client.callTool({
            name: 'browser_navigate',
